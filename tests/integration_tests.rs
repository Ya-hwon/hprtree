...
use hprtree::{BBox, HPRTreeBuilder, Point};
...
fn random_contain_test() {
    ...
    assert!(query_top.len() + query_left.len() + query_bottom.len() + query_right.len() == N_EXCLUDED);

    for i in query_top.into_iter().chain(query_left.into_iter()).chain(query_bottom.into_iter()).chain(query_right.into_iter()) {
        assert!(!i);
    }
}
